...
describe('Cell', () => {
    ...
    describe('Possibility computation', () => {
        ...
        it('possibilities should be calculated correctly', () => {
            ...
            const checks: Array<[[number, number], Array<number>]> = [
            ...
            for (const check of checks) {
                assert.deepEqual(grid[check[0][0]][check[0][1]].possibilities, check[1],
                    `Possibilities on cell (${check[0].join(', ')}) are incorrect.`);
            }
        });
    });
